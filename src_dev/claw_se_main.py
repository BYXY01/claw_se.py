"""Claw_SE development entry (dev-only runner; the single-file claw_se.py is built at ladder 4).
...
5. Build the secure main agent via factory.build_agent (the only seam).
6. Register the terminal channel, mount the input guard, run the MsgIO loop.
...
logger = logging.getLogger("Claw_SE.main")
...
def main() -> None:
    """Entry point: boot security, build agent, run the MsgIO loop."""
