...
Claw 系列有一个共同的起源：26 年初 OpenClaw 爆火，同时我们开始接触 LangChain 框架，展开一系列实验。其中一次测试把**终端当 `@tool` 接入 Agent**，产生了让人惊讶的效果——程序在自我修改迭代，仅仅 2 天就生长出文件、进程、自我感知等一系列功能，越来越接近 AI-cli 工具，于是启发了两个方向：

- **claw_se**——功能确定的安全版：追求确定性与安全，开箱即用。
- **Claw_EE**——种子版：几十行代码，自读/自改/自重启，能长出任意功能。
...
当前这是 **0.0.101**。SE = Small + Security（也是 Simple / Smart / Shield）。

**它来自那颗种子，但被设置为"安全"目标：这是一个功能固定的安全工具，不会自我修改。** 发布的是"当前安全的实验品"，而非"安全产品"。
...
- **26 年初**——OpenClaw 爆火，开始接触 LangChain：从最简非流式 agent 起步，再接上工具（天气、计算器），最后把**终端当 `@tool` 接入 Agent**——程序开始自我修改迭代，仅仅 2 天就生长出文件、进程、自我感知等功能，越来越接近 AI-cli 工具。
...
- **路线图**——多通道（已完成）→ 插件加载器 → 真实渠道 + 供应商故障转移 → OpenClaw 桥 → 会话内记忆。
...
- **OpenClaw**——启发与对标对象。
- **LangChain**——框架基础（Tool/AgentExecutor、LangGraph）。
- 上课的样例代码——一切的起点。
